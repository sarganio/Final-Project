...
using std::string;
	...
	public:
		...
		TcpClient(string myIP, unsigned short myPort, unsigned short toPort, string hostname, unsigned char messBuffer[MAX_MESSAGE_SIZE]);//,mutex& m);
		...
		*/
		int connect(unsigned short port, std::string hostname, unsigned char messBuffer[MAX_MESSAGE_SIZE]);//, mutex& m);
	};
