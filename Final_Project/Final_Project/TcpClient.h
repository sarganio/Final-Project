...
using std::string;
	...
	public:
		...
		TcpClient(string myIP, unsigned short myPort, unsigned short toPort, string hostname, Message* mess);//,mutex& m);
		...
		*/
		int connect(unsigned short port, std::string hostname, Message* mess);//, mutex& m);
	};
